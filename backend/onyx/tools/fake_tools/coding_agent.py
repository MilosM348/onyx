...
from onyx.coding_agent.utils import check_special_tool_calls
...
def _setup_session(
    ...
    """Download the repo, create a session, and extract the tarball.

    Returns the session id. Caller is responsible for delete_session.
    """
    ...
    # GitHub tarballs always have exactly one top-level dir; --strip-components=1
    # extracts the contents directly into cwd so the agent's bash calls see the
    # repo root immediately.
    extract_cmd = (
        ...
        try:
            ...
            with CodeInterpreterClient() as client:
                session_id = _setup_session(
                    client=client, repo=repo, github_token=github_token
                )
                try:
                    bash_tool = BashTool(
                    ...
                    while cycle_count < MAX_CODING_AGENT_CYCLES:
                        ...
                        assistant_with_tools = ChatMessageSimple(
                            message="",
                            token_count=sum(
                                tcs.token_count for tcs in tool_calls_simple
                            ),
                            message_type=MessageType.ASSISTANT,
                    ...
                    emitter.emit(
                        Packet(
                            placement=Placement(
                                turn_index=turn_index, tab_index=tab_index
                            ),
                    ...
                    try:
                        ...
                        logger.warning(
                            "Failed to delete coding agent session %s: %s",
                            session_id,
                            cleanup_exc,
                        )
